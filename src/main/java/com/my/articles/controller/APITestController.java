...
public class APITestController {
    ...
    public LoginDTO getResponse() {
        return new LoginDTO("eomssi", "1234");
    }

    @PostMapping("/apiPostArrayTest")
    @ResponseBody
    public Map<String, String> apiPostArrayTest(@RequestBody List<LoginDTO> loginDTO) {
        System.out.println(loginDTO.toString());

        Map<String, String> map = new HashMap<>();
        map.put("userId", loginDTO.get(0).getUserId());
        map.put("password", loginDTO.get(0).getPassword());
